...
async def _run_existing_url_check(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert report.workflow_prompt_id == "prompt:chemspace_workflow"
...
def test_readiness_check_entrypoint(tmp_path, monkeypatch, capsys):
    ...
    assert result == 0
    assert "ChemSpace MCP readiness check passed" in captured.out
    assert "server_instructions: ok" in captured.out
    assert "tool_annotations: ok" in captured.out
    assert "workflow_prompt: ok (prompt:chemspace_workflow)" in captured.out
    assert "search_fetch: ok" in captured.out
    assert "chatgpt_connector_name: ChemSpace Copilot" in captured.out
...
def test_readiness_check_json_output(tmp_path, monkeypatch, capsys):
    ...
    assert "fetch prompt:chemspace_workflow" in payload["chatgpt_smoke_prompt"]
